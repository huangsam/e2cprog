...
public class Scan {
	...
	/*
	 * private Token ccase2(char c1, char c2, TK r1, TK r2) { // the next char
	 * must be c2 otherwise it's an error. c = getchar(); if (c == c2) { return
	 * new Token(r2, String.valueOf(c1) + String.valueOf(c2), linenumber); }
	 * else if (myisdigit((char) c)) { String number = buildNUM(); putback =
	 * false; return new Token(r1, number, linenumber); } else {
	 * System.err.println("scan: got got " + c1 + " missing " + c2 +
	 * " (got ASCII " + c + ")"); return new Token(TK.ERROR, "bad ccase2",
	 * linenumber); } }
	 */
	...
	private String buildSTR() {
		...
		do {
			...
			k++;
			c = getchar();
		} while (c != '\"' && c != '\n' && c != EOF); // until right-hand DQUOTE
		c = getchar(); // take away DQUOTE
		putback = true;
		if (c != '\n' && c != ' ') {
			System.err
					.println("scan: string missing closing \"; quitting near "
							+ str);
			System.exit(1);
		}
		// Maybe we can check if the string is missing a
		// " if it detects a newline before the "
